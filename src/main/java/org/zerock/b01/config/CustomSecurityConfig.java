...
public class CustomSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.exceptionHandling(httpSecurityExceptionHandlingConfigurer -> {
            ...
            httpSecurityExceptionHandlingConfigurer.accessDeniedHandler(accessDeniedHandler());     //() 안에 accessDeniedHandler()는 Bean 객체.
        });

        //  social 정보를 /member/login 으로 넘겨서 작업
        http.oauth2Login(httpSecurityOAuth2LoginConfigurer -> {
            httpSecurityOAuth2LoginConfigurer.loginPage("/member/login");
            httpSecurityOAuth2LoginConfigurer.successHandler(authenticationSuccessHandler());   //  로그인 성공 시 authenticationSuccessHandler 동작
        });
